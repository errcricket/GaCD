...
nrow(features_2)
#kept_feature_column <- grep(c('*mean*', '*std*'), features_2$V2, ignore.case=T)
kept_feature_column <- grep('.*mean.*|.*std.*', features_2$V2, ignore.case=T)
#kept_feature_column
kept <- factor(features_2$V2[kept_feature_column])
length(kept)
names(kept)
kept <- levels(droplevels(features_2$V2[kept_feature_column]))
length(kept)
kept
#length(kept_feature_column)
#kept_features_2 <- which(features_2$V2 %in% kept_feature_column)
#print(kept_features_2)
